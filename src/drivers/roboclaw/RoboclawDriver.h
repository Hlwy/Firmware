...
private:
	...
	struct timeval _uart_timeout;
     static uint16_t calcCRC(const uint8_t *buf, size_t n, uint16_t init = 0);
	...
	uint8_t Read1(uint8_t address,uint8_t cmd,bool *valid);
     ...
     uint16_t crc_get();
public:
     int init_serial(const char *port, uint8_t address, int baud, uint32_t tout, bool doack = false);
     int open_serial_port(const char *port, int baud);
     void flush();
     int available();
     int transaction(e_command cmd, uint8_t *wbuff, size_t wbytes, uint8_t *rbuff, size_t rbytes, bool send_checksum = true, bool recv_checksum = false);

     RoboclawDriver();
	RoboclawDriver(const char *port, uint8_t address, int baud, uint32_t tout, bool doack = false);
	~RoboclawDriver();
